...
* [`eslint-node-test`](https://github.com/sindresorhus/eslint-node-test) (`configs.recommended`; upstream currently requires Node.js >=22 and ESLint >=10.4)
...
* :mute: [`package-json/require-exports`](https://github.com/michaelfaith/eslint-plugin-package-json/blob/main/docs/rules/require-exports.md) – *deactivated* – bin-only CLI packages legitimately ship without `exports`; too noisy across the canary fleet to surface useful signal
* :mute: [`package-json/no-redundant-files`](https://github.com/michaelfaith/eslint-plugin-package-json/blob/main/docs/rules/no-redundant-files.md) – *deactivated* – conflicts with `n/no-unpublished-bin`: npm auto-includes files listed in `bin`/`main`/`man` regardless of the `files` field, but eslint-plugin-n reports a false positive when those entries are removed from `files`, so fixing one triggers the other.
